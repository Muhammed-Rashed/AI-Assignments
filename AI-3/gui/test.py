...
class Game:
    ...
    def get_ai_move(self):
        ...
        result = janus.query_once(
            "ai_move(Board, Turn, Depth, Width, NewBoard, State)",
            {
                "Board": board,
                "Turn": ai_turn,
                "Depth": depth,
        ...
        if result and "NewBoard" in result:
            ...
            new_state = result.get("State", "ongoing")
    ...
    def handle_click(self, pos):
        ...
        if self.state == "ai_menu":
            if self.btn_easy.collidepoint(pos):
                ...
                self.ai_difficulty = 2 # depth, medium
                self.state = "role_menu"
            elif self.btn_hard.collidepoint(pos):
                self.ai_difficulty = 3 # depth, hard
                self.state = "role_menu"
            return
        ...
        if result and "NewBoard" in result:
            ...
            # APPLY MOVE DIRECTLY (no animation)
        ...
        while True:
            ...
            if self.state == "menu":
                ...
                if self.end_timer:
                    ...
                    if current_time - self.end_timer >= 1500:  # 1.5 seconds
                        self.show_end_screen = True
                        self.state = "game_over"
                screen.fill((255,255,255))
                self.update_animation()
                # AI move
                if self.state == "game" and self.game_state == "ongoing":
                    if self.player_role is not None:
                        ...
                        if is_ai_turn:
                            ai_board, ai_state = self.get_ai_move()

                            if ai_board:
                                print("APPLYING AI MOVE")

                                self.board = ai_board
                                self.game_state = ai_state
